...
- Reads full issue context: title, description, comments, attachments, acceptance criteria, linked
  docs, logs, screenshots, and related tickets.
- Classifies the issue type and separates expected behavior, actual behavior, facts, assumptions,
  and unknowns.
- Investigates code, configuration, data, CI, deployments, environments, and reproduction evidence
  where available.
- Produces a practical next-action recommendation before implementation begins.

## Collaborates with

- [`software-engineer`](../../) for technical code analysis, implementation feasibility, and
  validation planning.
- [`code-reviewer`](../code-reviewer/) after a fix or implementation exists.
- [`product-owner`](../../../product-owner/) when intended behavior or acceptance criteria are
  unclear.
