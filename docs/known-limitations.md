...
- **Nested skill support is tool-dependent.** Some agents may not automatically discover or follow
  `software-engineer/skills/issue-investigator` and `software-engineer/skills/code-reviewer`. In
  those cases, attach or reference the nested `SKILL.md` directly.
- **External access depends on your tools and permissions.** Jira, GitHub, Confluence, CI, logs, and
  cloud environments are only available when your agent session has the right credentials and
  network access.
...
- **Evidence-pack and cache flows depend on the execution environment.** Local-workspace mode,
  in-repo/cloud-agent mode, ephemeral runners, and web agents may differ in what files they can
  read, write, or preserve between turns.
